(() => {
  ...
  function renderProductRow(order, product) {
    ...
    const perPerson = product.price / product.count;
  ...
  function renderProductForm(order, initial) {
    ...
    function priceInfo() {
      ...
      const countValid = countNum > 0;
      const nameValid = name.trim().length > 0;
      const perPerson = priceValid && countValid ? priceNum / countNum : null;
      const canSave = nameValid && countValid && priceValid;
      return { priceNum, priceValid, countNum, countValid, nameValid, perPerson, canSave };
    }
    ...
    // --- Row 2: Anzahl (readonly) + Preis ---
    const row2 = document.createElement("div");
    row2.className = "row-2";

    const countField = document.createElement("div");
    countField.className = "field";
    countField.innerHTML = `<label class="label">Anzahl Personen</label>`;
    const countDisplay = document.createElement("div");
    countDisplay.className = "count-display";
    countField.appendChild(countDisplay);
    const countErr = document.createElement("div");
    countErr.className = "err";
    countErr.style.display = "none";
    countErr.textContent = "Mindestens eine Person auswählen.";
    countField.appendChild(countErr);
    row2.appendChild(countField);

    const priceField = document.createElement("div");
    priceField.className = "field";
    priceField.innerHTML = `<label class="label">Produktpreis ($)</label>`;
    ...
    preview.innerHTML = `<span>Preis pro Person</span><strong>—</strong>`;
    card.appendChild(preview);

    function updateCountAndPreview() {
      const info = priceInfo();
      countDisplay.textContent = info.countNum > 0 ? String(info.countNum) : "—";
      preview.querySelector("strong").textContent =
        info.perPerson !== null ? currency(info.perPerson) : "—";
    }
    updateCountAndPreview();
    ...
    function updateValidation() {
      ...
      countErr.style.display = info.countValid ? "none" : "block";
